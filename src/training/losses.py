...
def sobel_edges(x: torch.Tensor) -> torch.Tensor:
    ...
    if x.shape[1] == 3 and EDGE_GRAYSCALE:
        ...
        rgb_weights = torch.tensor([0.299, 0.587, 0.114], device=x.device).view(1, 3, 1, 1)
...
def vae_loss(recon_x: torch.Tensor, x: torch.Tensor, 
           ...
           beta: float = 1.0, reduction: str = 'mean') -> dict:
    """
...
def vae_geo_loss(recon_x: torch.Tensor, x: torch.Tensor, 
               mu: torch.Tensor, logvar: torch.Tensor,
               beta: float = 1.0, lambda_geo: float = 0.1) -> dict:
    """
